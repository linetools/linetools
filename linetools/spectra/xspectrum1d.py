...
class XSpectrum1D(object):
    ...
    def __init__(self, wave, flux, sig=None, co=None, units=None, select=0,
        ...
        if sig is not None:
            self._data['sig'] = np.reshape(sig, (self.nspec, self.npix))
            self.flag_sig = True
        else:
            self.flag_sig = False
        if co is not None:
            self._data['co'] = np.reshape(co, (self.nspec, self.npix))
            self.flag_co = True
        else:
            self._data['co'] = 1.
            self.flag_co = False
    ...
    def wavelength(self):
        ...
        """
        return self._data[self.select]['wave'] * self.units['wave']
    ...
    def sig(self):
        ...
        if self.flag_sig is False:
            ...
            return None
        #
        sig = self._data[self.select]['sig'] * self.units['flux']
        if self.normed and self.flag_co:
            sig /= self._data[self.select]['co']
        return sig
    ...
    def flux(self):
        ...
        """
        flux =  self._data[self.select]['flux'] * self.units['flux']
        if self.normed and self.flag_co:
            flux /= self._data[self.select]['co']
        return flux
    ...
    def co(self):
        ...
        return self._data[self.select]['co'] * self.units['flux']
