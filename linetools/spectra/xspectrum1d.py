    ...
    def normalize(self, conti=None, verbose=False, no_check=False):
        ...
        if conti is None:
            if hasattr(self, 'co') and self.co is not None:
                ...
                raise ValueError(
                    'Must specify a continuum with conti keyword.')
...
""")
    ...
    def perturb_continuum(self, rel_var = 0.05):
        """
        Adds a perturbation to the continuum by adding noise to the contpoints.

        Parameters
        ----------
        rel_var : float, optional

            Relative variation w/r to its original value. This will be
            the standard deviation of a Normal distribution added to
            the y values of the contpoints.

        Returns
        -------
        Updates self.co with a perturbation. To reset the original continuum use
        reset_continuum()
